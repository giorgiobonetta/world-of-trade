...
(() => {
  ...
  /* Due regole che valgono per ogni esercizio numerico, applicate qui una
     volta invece che ricordate template per template.

     1. Se la risposta è negativa, il testo deve dirlo e la spiegazione deve
        mostrare il segno. Prima la spiegazione scriveva "= $720,000 loss"
        mentre l'unica risposta accettata era -720000: chi calcolava bene
        veniva segnato errore e poi leggeva la propria cifra nella spiegazione.

     2. Una risposta a cinque cifre arrotondata non può avere tolleranza zero.
        Il TCE vero è 31.333,33 e veniva accettato solo 31333. Un'unità di
        tolleranza è arrotondamento, non una risposta diversa. */
  const numeric = (prompt, answer, unit, why, tolerance = 0) => {
    let p = prompt, w = why;
    if (answer < 0) {
      if (!/negative/i.test(p)) p = p.replace(/\s*$/, '') + ' Enter it as a negative number.';
      /* La moltiplicazione dà la grandezza; il segno viene dal fatto che è una
         perdita. Tenerli separati insegna la convenzione invece di nasconderla,
         e lascia il calcolo mostrato aritmeticamente vero. */
      w = w.replace(/=\s*(−?\$?[\d.,]+)\s+((?:premium |basis |timing )?loss)\./i,
                    (_, v, k) => {
                      const q = String(v).replace(/^−/, '');
                      const qual = k.trim().replace(/\s*loss$/, '');
                      return `= ${q} of value lost, so a ${qual ? qual + ' ' : ''}P&L of −${q}.`;
                    });
    }
    const tol = tolerance || (Math.abs(answer) >= 1000 && Number.isInteger(answer) ? 1 : 0);
    return { type: 'numeric', prompt: p, answer, unit, tolerance: tol, why: w };
  };
  ...
  const worlds = [
    {
      ...
      concepts:[
        ...
        R => { const stocks=r(R,24,60,4), annual=r(R,180,420,20); const daily=annual/365, days=round(stocks/daily,1); return numeric(`Stocks are ${stocks} Mt and annual demand is ${annual} Mt. Approximate days of cover?`,days,'days',`${stocks} ÷ (${annual} ÷ 365) = ${days} days of cover. Cover is what the market has left before a disruption starts to bite: a deficit with thin cover moves violently on small news.`,.1); },
        ...
        R => { const dest=r(R,680,860,10), freight=r(R,25,55,5), handling=r(R,4,12); const nb=dest-freight-handling; return numeric(`Destination value is $${dest}/t. Freight $${freight}/t and handling $${handling}/t. What is the implied origin netback?`,nb,'$/t',`${dest} − ${freight} − ${handling} = $${nb}/t. The netback is what makes two destinations comparable. The same benchmark can pay very differently once the route is paid for.`); },
        ...
        R => { const product=r(R,700,900,10), feed=r(R,600,760,10), cost=r(R,25,55,5); const margin=product-feed-cost; return numeric(`Product value $${product}/t, feedstock $${feed}/t, processing cost $${cost}/t. Simple processing margin?`,margin,'$/t',`${product} − ${feed} − ${cost} = $${margin}/t. Processing margins are self-correcting: when they go negative, plants cut runs and both the feed and the product balance shift.`); },
    ...
    },
    {
      ...
      concepts:[
        R => { const sell=r(R,720,980,10), buy=sell-r(R,45,95,5), freight=r(R,15,35,5), finance=r(R,4,14); const m=sell-buy-freight-finance; return numeric(`Sell $${sell}/t, buy $${buy}/t, freight $${freight}/t, finance $${finance}/t. Net trading margin?`,m,'$/t',`${sell} − ${buy} − ${freight} − ${finance} = $${m}/t. The gap between the headline spread and this number is where most of the gross margin quietly goes.`); },
        ...
        R => { const margin=r(R,12,28), qty=r(R,20,80,10)*1000; return numeric(`Margin is $${margin}/t on ${qty.toLocaleString('en-US')} t. Gross contribution?`,margin*qty,'$',`${margin} × ${qty.toLocaleString('en-US')} = ${money(margin*qty)}. Small per-tonne differences matter because the tonnage is large: a dollar a tonne on a cargo is real money.`); },
        ...
        R => { const gross=r(R,30,65,5), claim=r(R,4,16,2), dem=r(R,2,10,2), fx=r(R,-5,5); const net=gross-claim-dem+fx; return numeric(`Gross margin $${gross}/t, quality claim $${claim}/t, demurrage $${dem}/t, FX impact ${fx>=0?'+':''}$${fx}/t. Final margin?`,net,'$/t',`${gross} − ${claim} − ${dem} ${fx>=0?'+':'−'} ${Math.abs(fx)} = $${net}/t. Claims, demurrage and currency arrive after the deal is agreed. Budgeting for them is what separates a quoted margin from a banked one.`); },
    ...
    },
    {
      ...
      concepts:[
        R => { const lump=r(R,450,1200,50)*1000, qty=r(R,20,60,5)*1000; const pt=round(lump/qty,2); return numeric(`Voyage freight is ${money(lump)} for ${qty.toLocaleString('en-US')} t. Freight per tonne?`,pt,'$/t',`${money(lump)} ÷ ${qty.toLocaleString('en-US')} = $${pt}/t. Freight per tonne is the only form in which it can be compared with the margin. Lumpsum hides how the cost moves with the quantity loaded.`,.02); },
        ...
        R => { const excess=r(R,1,5), rate=r(R,12,30,2)*1000; return numeric(`${excess} days exceed allowed laytime. Demurrage is ${money(rate)}/day. Cost?`,excess*rate,'$',`${excess} × ${money(rate)} = ${money(excess*rate)}. Demurrage scales with hours, not tonnes: a small parcel on a slow berth can carry more of it than a large one loaded fast.`); },
        ...
        R => { const hire=r(R,14,28,2)*1000, days=r(R,20,45,5), bunkers=r(R,250,650,50)*1000; const cost=hire*days+bunkers; return numeric(`Time-charter hire ${money(hire)}/day for ${days} days plus bunkers ${money(bunkers)}. Approximate voyage vessel cost?`,cost,'$',`${money(hire)} × ${days} + ${money(bunkers)} = ${money(cost)}. Under a time charter the clock is yours from delivery. Hire plus bunkers is the true cost of the voyage, and there is no demurrage to claim.`); },
    ...
    },
    {
      ...
      concepts:[
        R => { const value=r(R,2,10)*1000000, days=r(R,30,120,15), rate=one(R,[5,6,7,8]); const interest=round(value*(rate/100)*(days/360)); return numeric(`You fund ${money(value)} of cargo for ${days} days at ${rate}% a year, on a 360-day basis. What is the interest cost?`,interest,'$',`${money(value)} × ${rate}% × ${days}/360 ≈ ${money(interest)}. Every day of the working capital gap is financed. On thin margins, payment terms can matter as much as price.`,2); },
        ...
        R => { const inv=r(R,4,12)*1000000, advance=one(R,[70,75,80,85,90]); const loan=inv*advance/100; return numeric(`Eligible inventory is ${money(inv)} and borrowing-base advance rate is ${advance}%. Maximum borrowing against it?`,loan,'$',`${money(inv)} × ${advance}% = ${money(loan)}. The advance rate is what actually funds the volume. When prices fall the base shrinks exactly when cash is needed most.`); },
        ...
        R => { const margin=r(R,20,50,5), value=r(R,700,900,20), extra=r(R,30,90,15), rate=one(R,[6,7,8]); const cost=round(value*(rate/100)*(extra/360),2); return numeric(`A sale earns $${margin}/t but adds ${extra} days of financing on $${value}/t at ${rate}% p.a. Incremental finance cost per tonne?`,cost,'$/t',`${value} × ${rate}% × ${extra}/360 ≈ $${cost}/t. Longer payment terms are never free: compare this figure with the margin before agreeing to them.`,.02); },
    ...
    },
    {
      ...
      concepts:[
        R => { const phys=r(R,60,140,10)*1000, short=r(R,20,100,10)*1000; return numeric(`Long physical ${phys.toLocaleString('en-US')} t and short futures ${short.toLocaleString('en-US')} t. Net flat-price exposure?`,phys-short,'t',`${phys.toLocaleString('en-US')} − ${short.toLocaleString('en-US')} = ${(phys-short).toLocaleString('en-US')} t net long. Only the net moves with the outright price. It is the first number a risk manager asks for, and the one limits are checked against.`); },
    ...
    },
    {
      ...
      concepts:[
        ...
        R => { const qty=r(R,10,40,5)*1000, premium=r(R,3,12), returnVal=qty*premium; return numeric(`Put premium is $${premium}/t on ${qty.toLocaleString('en-US')} t. Upfront option premium cost?`,returnVal,'$',`${qty.toLocaleString('en-US')} × $${premium} = ${money(returnVal)}. The premium is paid now whether or not the option is ever exercised. That known, limited cost is what you are buying.`); },
    ...
    },
    {
      ...
      concepts:[
        R => { const lme=r(R,8000,10000,100), prem=r(R,100,350,25); return numeric(`Copper LME is $${lme}/t and physical premium is $${prem}/t. All-in metal price?`,lme+prem,'$/t',`${lme} + ${prem} = $${lme+prem}/t. Only part of this price is hedgeable on the exchange for most metals: the premium is a separate exposure you keep.`); },
        ...
        R => { const tonnes=r(R,5,20)*1000, tc=r(R,40,100,10); return numeric(`A concentrate contract has a treatment charge of $${tc}/dry metric tonne on ${tonnes.toLocaleString('en-US')} dmt. Gross TC amount?`,tonnes*tc,'$',`${tonnes.toLocaleString('en-US')} × $${tc} = ${money(tonnes*tc)}. Treatment charges are the smelter's margin and the miner's cost. They move with concentrate availability and decide whether the trade works.`); },
        ...
        R => { const qty=r(R,2,10)*1000, premBuy=r(R,80,160,10), premSell=premBuy+r(R,40,100,10), freight=r(R,15,35,5); return numeric(`Buy LME + $${premBuy}/t, sell LME + $${premSell}/t, freight $${freight}/t. Premium margin?`,premSell-premBuy-freight,'$/t',`${premSell} − ${premBuy} − ${freight} = $${premSell-premBuy-freight}/t. The premium is where a metals desk earns. It moves for reasons the exchange screen never shows.`); },
    ...
    },
    {
      ...
      concepts:[
        R => { const brent=r(R,65,95), diff=r(R,-8,5); return numeric(`Brent is $${brent}/bbl and a crude trades Brent ${diff>=0?'+':''}$${diff}/bbl. Crude price?`,brent+diff,'$/bbl',`${brent} ${diff>=0?'+':'−'} ${Math.abs(diff)} = $${brent+diff}/bbl. The benchmark carries the flat price; the differential carries everything specific about that barrel.`); },
        R => { const crude=r(R,65,95), cost=r(R,4,10), margin=r(R,5,18); const product=crude+cost+margin; return numeric(`Product value $${product}/bbl, crude $${crude}/bbl, variable refining cost $${cost}/bbl. Simple gross product-minus-feed margin?`,margin,'$/bbl',`${product} − ${crude} − ${cost} = $${margin}/bbl. Refining margins are the signal that pulls crude demand and pushes product supply at the same time.`); },
    ...
    },
    {
      ...
      concepts:[
        R => { const ttf=r(R,25,55), basis=r(R,-6,8); return numeric(`TTF is €${ttf}/MWh and a contract is TTF ${basis>=0?'+':''}€${basis}/MWh. Contract price?`,ttf+basis,'€/MWh',`${ttf} ${basis>=0?'+':'−'} ${Math.abs(basis)} = €${ttf+basis}/MWh. A formula price and a hub price answer different questions. The gap between them is where contract renegotiations come from.`); },
        R => { const dest=r(R,12,22), shipping=r(R,1,4), regas=r(R,0.5,1.5,.5); const nb=round(dest-shipping-regas,1); return numeric(`Destination LNG value $${dest}/MMBtu, shipping $${shipping}/MMBtu, regas $${regas}/MMBtu. Netback before source cost?`,nb,'$/MMBtu',`${dest} − ${shipping} − ${regas} = $${nb}/MMBtu. Two indices are not comparable until the whole route is paid for. This is the number that decides where the cargo goes.`,.05); },
        ...
        R => { const qty=r(R,2,4)*1000000, spread=r(R,1,4); return numeric(`A ${qty.toLocaleString('en-US')} MMBtu cargo has a destination spread of $${spread}/MMBtu before incremental logistics. Gross spread value?`,qty*spread,'$',`${qty.toLocaleString('en-US')} × $${spread} = ${money(qty*spread)}. LNG cargo sizes make small unit moves very large. A few cents per MMBtu is a material P&L on one ship.`); },
    ...
    },
    {
      ...
      concepts:[
        R => { const mw=r(R,10,100,10), hours=r(R,12,48,12); return numeric(`${mw} MW delivered flat for ${hours} hours equals how many MWh?`,mw*hours,'MWh',`${mw} × ${hours} = ${mw*hours} MWh. Power is sold in energy but delivered as power over time. Getting this conversion wrong misprices the whole contract.`); },
        ...
        R => { const power=r(R,80,140,10), gas=r(R,25,55,5), heat=2, carbon=r(R,5,20,5); return numeric(`Power €${power}/MWh, gas €${gas}/MWh(th), heat rate ${heat} MWh(th)/MWh(e), carbon/other €${carbon}/MWh. Simple spark margin?`,power-gas*heat-carbon,'€/MWh',`${power} − ${gas}×${heat} − ${carbon} = €${power-gas*heat-carbon}/MWh. The spark spread is the generation margin, and like a refining crack it is a spread you can hedge directly.`); },
        ...
        R => { const dev=r(R,5,25), hours=r(R,2,8), price=r(R,100,250,25); return numeric(`You are short ${dev} MW for ${hours} hours and imbalance power costs €${price}/MWh. Gross imbalance cost?`,dev*hours*price,'€',`${dev} × ${hours} × €${price} = €${(dev*hours*price).toLocaleString('en-US')} cost. Imbalance is what you pay for being wrong about volume, and it is priced to hurt. It is why shaping the hedge matters.`); },
    ...
    },
    {
      ...
      concepts:[
        ...
        R => { const fut=r(R,220,320,10), basis=r(R,-25,25,5); return numeric(`Futures are $${fut}/t and local basis is ${basis>=0?'+':''}$${basis}/t. Physical cash price?`,fut+basis,'$/t',`${fut} ${basis>=0?'+':'−'} ${Math.abs(basis)} = $${fut+basis}/t. Futures price the benchmark; the basis is the local market. On grains that difference is usually where the result comes from.`); },
        R => { const spread=r(R,8,25), storage=r(R,3,10), finance=r(R,2,8); return numeric(`Deferred premium $${spread}/t, storage $${storage}/t, finance $${finance}/t. Net carry before losses?`,spread-storage-finance,'$/t',`${spread} − ${storage} − ${finance} = $${spread-storage-finance}/t. The forward spread only pays if it is wider than the full carry, and the carry moves with the flat price. Storage only pays if the deferred premium beats the full carry. A negative result says the market is not paying you to hold it.`); },
    ...
    },
    {
      ...
      concepts:[
        ...
        R => { const annual=r(R,50,200,25)*1000, margin=r(R,5,20); return numeric(`An offtake supplies ${annual.toLocaleString('en-US')} t/year and expected contribution is $${margin}/t. Annual gross contribution?`,annual*margin,'$',`${annual.toLocaleString('en-US')} × $${margin} = ${money(annual*margin)}. Structural flow is valued on the year, not on one cargo. It is what supports financing and freight programmes.`); },
    ...
    },
    {
      ...
      concepts:[
        ...
        R => { const base=r(R,20,80,10)*1000, tol=one(R,[5,10]); const max=base*(1+tol/100); return numeric(`Contract quantity is ${base.toLocaleString('en-US')} t with ${tol}% seller option. Maximum contractual quantity?`,max,'t',`${base.toLocaleString('en-US')} × (1 + ${tol}%) = ${max.toLocaleString('en-US')} t. Every tolerance belongs to someone. If the contract does not say the choice is yours, assume it is being used against you.`); },
        R => { const total=r(R,30,90,10)*1000, done=r(R,10,total/1000-10,10)*1000; return numeric(`A programme requires ${total.toLocaleString('en-US')} t this month. ${done.toLocaleString('en-US')} t has already loaded. Remaining scheduled volume?`,total-done,'t',`${total.toLocaleString('en-US')} − ${done.toLocaleString('en-US')} = ${(total-done).toLocaleString('en-US')} t. Scheduling is the difference between a contract and a delivery. What is left to load is what still carries risk.`); },
        ...
        R => { const qty=r(R,20,70,10)*1000, rate=r(R,4,10)*1000; const hours=round(qty/rate,1); return numeric(`Terminal loading rate is ${rate.toLocaleString('en-US')} t/hour for a ${qty.toLocaleString('en-US')} t parcel. Approximate loading hours?`,hours,'hours',`${qty.toLocaleString('en-US')} ÷ ${rate.toLocaleString('en-US')} = ${hours} hours. Loading rate sets laytime, and laytime sets demurrage. It is the first thing to check before agreeing a laycan.`,.1); },
        R => { const nominated=r(R,40,90,10)*1000, accepted=nominated-r(R,5,20,5)*1000; return numeric(`You nominate ${nominated.toLocaleString('en-US')} t but the system confirms only ${accepted.toLocaleString('en-US')} t. Unscheduled shortfall?`,nominated-accepted,'t',`${nominated.toLocaleString('en-US')} − ${accepted.toLocaleString('en-US')} = ${(nominated-accepted).toLocaleString('en-US')} t. A confirmed shortfall is a commercial problem, not a paperwork one: it changes freight per tonne and may trigger deadfreight.`); },
        ...
        R => { const doc=r(R,20,80,5)*1000, actual=doc-r(R,100,900,100); return numeric(`Shipping documents show ${doc.toLocaleString('en-US')} t while independently confirmed loaded quantity is ${actual.toLocaleString('en-US')} t. Quantity difference?`,doc-actual,'t',`${doc.toLocaleString('en-US')} − ${actual.toLocaleString('en-US')} = ${(doc-actual).toLocaleString('en-US')} t. Documents and measured quantity are not the same thing. Which one is binding is decided by the contract, before shipment.`); },
        R => { const qty=r(R,10,50,5)*1000, price=r(R,500,950,25); return numeric(`Final invoiced quantity is ${qty.toLocaleString('en-US')} t at $${price}/t. Invoice value before taxes/adjustments?`,qty*price,'$',`${qty.toLocaleString('en-US')} × $${price} = ${money(qty*price)}. The invoice is what gets paid against. Any difference from the measured quantity has to be agreed before it is issued.`); },
        ...
        R => { const book=r(R,20,80,5)*1000, variance=r(R,50,500,50); const phys=book-variance; return numeric(`Book inventory is ${book.toLocaleString('en-US')} t; physical measurement is ${phys.toLocaleString('en-US')} t. Reconciliation shortage?`,book-phys,'t',`${book.toLocaleString('en-US')} − ${phys.toLocaleString('en-US')} = ${book-phys} t. A reconciliation gap is either a measurement issue or a loss. Treating it as paperwork is how losses stay hidden.`); },
        R => { const opening=r(R,20,60,5)*1000, receipts=r(R,10,40,5)*1000, deliveries=r(R,10,40,5)*1000; const close=opening+receipts-deliveries; return numeric(`Opening stock ${opening.toLocaleString('en-US')} t, receipts ${receipts.toLocaleString('en-US')} t, deliveries ${deliveries.toLocaleString('en-US')} t. Expected closing stock?`,close,'t',`${opening.toLocaleString('en-US')} + ${receipts.toLocaleString('en-US')} − ${deliveries.toLocaleString('en-US')} = ${close.toLocaleString('en-US')} t. Expected closing stock is what the physical count is checked against. A gap that is not explained is a claim waiting to happen.`); },
        ...
        R => { const qty=r(R,15,60,5)*1000, short=one(R,[0.2,0.4,0.6,0.8]); const tonnes=round(qty*short/100,1); return numeric(`A ${qty.toLocaleString('en-US')} t cargo has a confirmed ${short}% quantity shortage versus contract. Short tonnes?`,tonnes,'t',`${qty.toLocaleString('en-US')} × ${short}% = ${tonnes} t. Shortage is measured against contract quantity, and whether you can claim it depends on the franchise and the binding certificate.`,.1); },
        R => { const affected=r(R,5,30,5)*1000, discount=r(R,4,18,2); return numeric(`${affected.toLocaleString('en-US')} t is accepted subject to a $${discount}/t quality allowance. Gross claim value?`,affected*discount,'$',`${affected.toLocaleString('en-US')} × $${discount} = ${money(affected*discount)}. Quality allowances are decided by the words agreed beforehand, not by who is more certain on arrival.`); },
    ...
    },
    {
      ...
      concepts:[
        ...
        R => { const qty=r(R,10,80,10)*1000, trade=r(R,500,900,25), mark=trade+r(R,-40,40,10); const pnl=(mark-trade)*qty; return numeric(`Long position ${qty.toLocaleString('en-US')} t entered at $${trade}/t is marked at $${mark}/t. Simple mark-to-market P&L?`,pnl,'$',`($${mark} − $${trade}) × ${qty.toLocaleString('en-US')} = ${money(pnl)}. Mark-to-market makes an unrealised loss visible before it is realised, and it is what a margin call is calculated on.`); },
        R => { const price=r(R,-3,8)*100000, freight=r(R,-2,5)*100000, fx=r(R,-2,3)*100000; const total=price+freight+fx; return numeric(`Daily attribution: price ${money(price)}, freight ${money(freight)}, FX ${money(fx)}. Explained P&L?`,total,'$',`${money(price)} + ${money(freight)} + ${money(fx)} = ${money(total)}. Attribution is what makes a P&L believable. A number nobody can explain is a number nobody can defend.`); },
        ...
        R => { const phys=r(R,40,120,10)*1000, paper=r(R,20,100,10)*1000; return numeric(`Long physical ${phys.toLocaleString('en-US')} t and short paper ${paper.toLocaleString('en-US')} t in the same pricing bucket. Net exposure?`,phys-paper,'t',`${phys.toLocaleString('en-US')} − ${paper.toLocaleString('en-US')} = ${(phys-paper).toLocaleString('en-US')} t. Matching volumes is not matching risk. Only the net in the same pricing bucket is genuinely offset.`); },
        ...
        R => { const old=r(R,20,80,10)*1000, change=r(R,5,20,5)*1000; return numeric(`Booked purchase quantity is ${old.toLocaleString('en-US')} t and an authorised amendment increases it by ${change.toLocaleString('en-US')} t. New booked quantity?`,old+change,'t',`${old.toLocaleString('en-US')} + ${change.toLocaleString('en-US')} = ${(old+change).toLocaleString('en-US')} t. An amendment changes the exposure the moment it is authorised. If the book does not follow, the hedge is against the wrong quantity.`); },
        R => { const qty=r(R,10,50,5)*1000, old=r(R,600,900,25), amend=old+r(R,-20,30,10); const impact=(amend-old)*qty; return numeric(`A fixed-price term on ${qty.toLocaleString('en-US')} t is amended from $${old}/t to $${amend}/t. Seller-side gross value change?`,impact,'$',`($${amend} − $${old}) × ${qty.toLocaleString('en-US')} = ${money(impact)}. A price amendment moves value between the two sides. It has to reach the book and the hedge, not just the contract file.`); },
        ...
        R => { const m1=r(R,70,100,5), m3=m1+r(R,-10,15,5); const mid=round((m1+m3)/2,1); return numeric(`Approved marks are $${m1}/t for month 1 and $${m3}/t for month 3. Under simple linear interpolation, approximate month-2 mark?`,mid,'$/t',`($${m1} + $${m3}) ÷ 2 = $${mid}/t. Interpolated marks are an assumption, not an observation. They are where valuation disputes start.`,.1); },
        R => { const qty=r(R,10,60,10)*1000, desk=r(R,600,900,25), independent=desk-r(R,2,12,2); const diff=(desk-independent)*qty; return numeric(`Desk mark is $${desk}/t; independent control mark is $${independent}/t on ${qty.toLocaleString('en-US')} t. Valuation difference?`,diff,'$',`($${desk} − $${independent}) × ${qty.toLocaleString('en-US')} = ${money(diff)}. An independent mark exists precisely so the desk does not value its own position. The difference is the number to explain.`); },
        ...
        R => { const report=r(R,2,10)*1000000, system=report-r(R,1,8)*10000; return numeric(`Desk report shows ${money(report)} P&L; controlled system shows ${money(system)}. Absolute break?`,Math.abs(report-system),'$',`|${money(report)} − ${money(system)}| = ${money(Math.abs(report-system))}. A break between the desk view and the controlled system means one of them is wrong, and until it is closed you do not know which.`); },
        R => { const breaks=r(R,2,8), each=r(R,10,50,5)*1000; return numeric(`${breaks} unresolved valuation breaks average ${money(each)} each in absolute value. Gross break inventory?`,breaks*each,'$',`${breaks} × ${money(each)} = ${money(breaks*each)}. Unresolved breaks accumulate. Their total is the amount of P&L nobody can currently stand behind.`); },
        ...
        R => { const reported=r(R,2,10)*1000000, explained=reported-r(R,50,400,50)*1000; return numeric(`Reported daily P&L is ${money(reported)} and explained P&L is ${money(explained)}. Unexplained residual?`,reported-explained,'$',`${money(reported)} − ${money(explained)} = ${money(reported-explained)}. The unexplained residual is the honest measure of how well the book is understood. Growing residuals precede unpleasant surprises.`); }
      ]
    },
    {
      ...
      concepts:[
        ...
        R => { const supplier=r(R,0,30,10), inventory=r(R,20,70,10), customer=r(R,20,90,10); const cycle=inventory+customer-supplier; return numeric(`Supplier credit ${supplier} days, inventory/transport ${inventory} days, customer collection ${customer} days after sale. Approximate cash conversion cycle?`,cycle,'days',`${inventory} + ${customer} − ${supplier} = ${cycle} days. The cash conversion cycle is how long your money is in someone else's business. It sets how much volume the desk can carry.`); },
        R => { const value=r(R,3,15)*1000000, days=r(R,30,120,15), rate=one(R,[5,6,7,8]); const cost=round(value*rate/100*days/360); return numeric(`${money(value)} is tied up for ${days} days at ${rate}% annual funding cost on a 360-day basis. Funding cost?`,cost,'$',`${money(value)} × ${rate}% × ${days}/360 ≈ ${money(cost)}. Funding cost scales with the value of the cargo, so the same terms cost more at a high flat price than at a low one.`,2); },
        ...
        R => { const lots=r(R,20,100,10), size=one(R,[25,50,100]), move=r(R,10,60,10); return numeric(`A short futures position has ${lots} lots of ${size} t. Price rises $${move}/t. Approximate variation-margin cash outflow before other effects?`,lots*size*move,'$',`${lots} × ${size} × $${move} = ${money(lots*size*move)}. Variation margin is cash today while the physical gain is unrealised. That gap has killed well-hedged companies.`); },
        ...
        R => { const usd=r(R,2,10)*1000000, eurusd=one(R,[1.05,1.08,1.10,1.12]); const eur=round(usd/eurusd,2); return numeric(`A USD payable is ${money(usd)} and EUR/USD is ${eurusd} USD per EUR. Approximate EUR needed to buy the USD?`,eur,'EUR',`${money(usd)} ÷ ${eurusd} ≈ €${Number(eur).toLocaleString('en-US')}. Currency is a real exposure on a physical desk, not an accounting detail. It should be hedged with the trade, not after it.`,2); },
        R => { const debt=r(R,5,25)*1000000, delta=one(R,[0.5,1.0,1.5,2.0]), days=90; const extra=round(debt*(delta/100)*days/360); return numeric(`Floating-rate debt ${money(debt)} reprices ${delta} percentage points higher for ${days} days. Approximate incremental interest?`,extra,'$',`${money(debt)} × ${delta}% × ${days}/360 ≈ ${money(extra)}. Floating debt reprices while the cargo does not. The financing leg has its own risk and its own hedge.`,2); },
        ...
        R => { const liquidity=r(R,15,60)*1000000, daily=r(R,1,5)*1000000; const days=round(liquidity/daily,1); return numeric(`Usable liquidity ${money(liquidity)} and stressed net cash burn ${money(daily)}/day. Simple survival horizon?`,days,'days',`${money(liquidity)} ÷ ${money(daily)} = ${days} days. Survival horizon is the question a treasurer actually asks: not whether the position is right, but how long you can hold it.`,.1); },
    ...
    },
    {
      ...
      concepts:[
        ...
        R => { const qty=r(R,10,80,10)*1000, price=r(R,400,1000,25); return numeric(`ETRM captures ${qty.toLocaleString('en-US')} t at $${price}/t. Gross trade notional?`,qty*price,'$',`${qty.toLocaleString('en-US')} × $${price} = ${money(qty*price)}. Notional is what the systems, the limits and the counterparty exposure are all measured against. Getting it wrong misstates all three.`); },
        R => { const barrels=r(R,100,900,100)*1000, bblPerT=one(R,[7.2,7.4,7.6]); const tonnes=round(barrels/bblPerT,1); return numeric(`A product mapping uses ${bblPerT} barrels per tonne. ${barrels.toLocaleString('en-US')} bbl corresponds to approximately how many tonnes?`,tonnes,'t',`${barrels.toLocaleString('en-US')} ÷ ${bblPerT} ≈ ${tonnes.toLocaleString('en-US')} t. Unit conversions are a routine source of real loss. A wrong density or barrel factor misprices the whole cargo.`,.1); },
        ...
        R => { const old=r(R,70,120,5), fresh=old+r(R,-10,15,5); return numeric(`Stored mark is $${old}/t and fresh approved mark is $${fresh}/t. Absolute price difference?`,Math.abs(fresh-old),'$/t',`|$${fresh} − $${old}| = $${Math.abs(fresh-old)}/t. A stale mark is a wrong valuation that looks authoritative. Refresh dates matter as much as the numbers.`); },
        R => { const px1=r(R,70,110,5), px2=px1+r(R,-4,6,2), qty=r(R,10,60,10)*1000; return numeric(`Vendor A marks $${px1}/t and Vendor B $${px2}/t on ${qty.toLocaleString('en-US')} t. Gross valuation difference between sources?`,Math.abs(px1-px2)*qty,'$',`|$${px1} − $${px2}| × ${qty.toLocaleString('en-US')} = ${money(Math.abs(px1-px2)*qty)}. Two sources disagreeing is normal; not knowing which one the book uses is not.`); },
        ...
        R => { const p1=r(R,60,100,5), p3=p1+r(R,-12,18,6); const p2=round((p1+p3)/2,1); return numeric(`Month-1 curve point $${p1}/t and month-3 point $${p3}/t. Simple linear month-2 interpolation?`,p2,'$/t',`($${p1} + $${p3}) ÷ 2 = $${p2}/t. Interpolation fills a gap in the curve with an assumption. It is acceptable if it is visible, dangerous if it is silent.`,.1); },
        R => { const days=one(R,[20,21,22]), sum=r(R,1400,2200,20); const avg=round(sum/days,2); return numeric(`The sum of ${days} daily settlement prices is ${sum}. Monthly average settlement price?`,avg,'$/unit',`${sum} ÷ ${days} = ${avg}. Averaged settlements are how quotational periods actually price. One missing day changes the answer.`,.02); },
        ...
        R => { const source=r(R,50,120,10)*1000, target=source-r(R,50,500,50); return numeric(`Source system position ${source.toLocaleString('en-US')} t; target system ${target.toLocaleString('en-US')} t. Interface break?`,source-target,'t',`${source.toLocaleString('en-US')} − ${target.toLocaleString('en-US')} = ${source-target} t. An interface break means two systems believe different things about the same position. Trading on either is a guess.`); },
        R => { const total=r(R,500,2000,100), failed=r(R,2,20); const rate=round(failed/total*100,2); return numeric(`${failed} of ${total} trade-interface messages failed and required repair. Failure rate?`,rate,'%',`${failed} ÷ ${total} × 100 = ${rate}%. Failure rates decide how much manual repair the desk carries, and manual repair is where operational losses come from.`,.02); },
        ...
        R => { const manual=r(R,20,80), automated=r(R,2,15), runs=r(R,20,100); const saved=(manual-automated)*runs; return numeric(`A control took ${manual} minutes manually and ${automated} minutes after automation, run ${runs} times per month. Monthly minutes saved?`,saved,'minutes',`(${manual} − ${automated}) × ${runs} = ${saved.toLocaleString('en-US')} minutes. Automation is justified by hours saved against error rates, not by the fact that it is automation.`); },
        R => { const rows=r(R,1000,10000,1000), bad=r(R,5,50,5); const clean=rows-bad; return numeric(`A market-data load contains ${rows.toLocaleString('en-US')} records; ${bad} fail validation. Records eligible to continue after quarantine?`,clean,'records',`${rows.toLocaleString('en-US')} − ${bad} = ${clean.toLocaleString('en-US')} records. Records that fail validation do not disappear: they are either repaired or they leave a gap in the position.`); }
      ]
    },
    {
      ...
      concepts:[
        ...
        R => { const entities=r(R,4,12), missing=r(R,1,3); return numeric(`A KYC file requires screening for ${entities} relevant entities/people and ${missing} remain unresolved. How many are currently cleared in this simplified workflow?`,entities-missing,'parties',`${entities} − ${missing} = ${entities-missing} parties. Screening is a repeated control, not a one-off. Anything unresolved is a trade you are not yet allowed to do.`); },
        ...
        R => { const checks=r(R,8,20), alerts=r(R,1,5); return numeric(`A transaction produces ${checks} screening checks and ${alerts} unresolved alerts. Checks with no unresolved alert?`,checks-alerts,'checks',`${checks} − ${alerts} = ${checks-alerts}. An alert that nobody closed is an open question about whether the trade is permitted at all.`); },
        R => { const value=r(R,2,10)*1000000; return choice(`A payment of ${money(value)} is placed on hold pending sanctions review. How much of it can you treat as available in the meantime?`,['None of it — the whole amount is unavailable until the hold is resolved','Half, pending review','Everything except a nominal reserve','It depends on the size of the margin'],0,`A hold is binary until the control process clears it. Assuming partial release because the cash is needed is exactly the reasoning a control exists to stop.`); },
        ...
        R => { const cargos=r(R,10,40), enhanced=r(R,2,8); const share=round(enhanced/cargos*100,1); return numeric(`${enhanced} of ${cargos} reviewed cargos require enhanced maritime due diligence. Share of reviewed cargos?`,share,'%',`${enhanced} ÷ ${cargos} × 100 = ${share}%. Enhanced due diligence is triggered by facts about the voyage, not by the size of the margin.`,.1); },
        ...
        R => { const contract=r(R,5,30)*1000000, fee=one(R,[1,2,3,4]); return numeric(`An intermediary fee is ${fee}% of a ${money(contract)} contract. Fee amount to be reviewed?`,contract*fee/100,'$',`${money(contract)} × ${fee}% = ${money(contract*fee/100)}. Intermediary fees are the classic route for improper payments. The size alone is a reason to look, not a reason to worry.`); },
        ...
        R => { const alerts=r(R,5,20), cleared=r(R,1,alerts-1); return numeric(`A review queue starts with ${alerts} alerts and ${cleared} are documented as resolved. Alerts still open?`,alerts-cleared,'alerts',`${alerts} − ${cleared} = ${alerts-cleared}. An open alert blocks the trade. The queue is not administration, it is the gate.`); },
        R => { const cases=r(R,20,80,10), overdue=r(R,1,10); const pct=round(overdue/cases*100,1); return numeric(`${overdue} of ${cases} due-diligence cases exceed the internal review target. Overdue share?`,pct,'%',`${overdue} ÷ ${cases} × 100 = ${pct}%. Cases past the internal target are the ones most likely to be waved through under pressure.`,.1); },
        ...
        R => { const total=r(R,20,60)*1000000, blocked=r(R,2,12)*1000000, cleared=r(R,1,8)*1000000; return numeric(`A transaction programme totals ${money(total)}. ${money(blocked)} is currently on compliance hold, and ${money(cleared)} of earlier holds has already been released. How much is on hold right now?`,blocked,'$',`${money(blocked)}. The released amount is history: it is not added back, and the programme total is not the question. Compliance reporting is a snapshot of what is blocked today, and mixing it with what was blocked last month is how a desk convinces itself it has more room than it does.`); },
        R => { const expected=r(R,30,100)*1000000, paused=r(R,5,25)*1000000; return numeric(`A programme expected ${money(expected)} of turnover, but ${money(paused)} is paused pending review. Turnover still eligible to proceed before other constraints?`,expected-paused,'$',`${money(expected)} − ${money(paused)} = ${money(expected-paused)}. Turnover paused pending review is turnover you cannot count. Compliance constrains the business before it constrains the trade.`); }
      ]
    },
    {
      ...
      concepts:[
        ...
        R => { const spread=r(R,8,28,2), storage=r(R,2,8), finance=r(R,2,7), loss=r(R,1,4); return numeric(`Deferred premium $${spread}/t; storage/handling $${storage}/t, finance $${finance}/t and expected loss $${loss}/t. Net carry value?`,spread-storage-finance-loss,'$/t',`${spread} − ${storage} − ${finance} − ${loss} = $${spread-storage-finance-loss}/t. Storage only pays if the deferred premium beats the full carry. A negative result says the market is not paying you to hold it.`); },
        R => { const cap=r(R,20,100,10)*1000, used=r(R,10,90,10); const volume=cap*used/100; return numeric(`Storage capacity ${cap.toLocaleString('en-US')} t is ${used}% utilised. Inventory in tank?`,volume,'t',`${cap.toLocaleString('en-US')} × ${used}% = ${volume.toLocaleString('en-US')} t. Tank utilisation decides what you can actually deliver and what you can still receive. Full tanks turn a price problem into a logistics one.`); },
        ...
        R => { const feed=r(R,600,900,25), yieldPct=one(R,[90,92,94,96]), product=r(R,680,1050,25), variable=r(R,20,60,5); const margin=round(product*yieldPct/100-feed-variable,1); return numeric(`Feed costs $${feed}/t. Saleable yield ${yieldPct}%, product value $${product}/t of product and variable conversion cost $${variable}/t feed. Approximate margin per tonne of feed?`,margin,'$/t feed',`$${product} × ${yieldPct}% − $${feed} − $${variable} = $${margin}/t feed. Yield is what converts a feed price into a product margin. A point of yield is often worth more than a dollar of feed.`,.1); },
        R => { const throughput=r(R,100,500,50)*1000, margin=r(R,5,30,5); return numeric(`A tolling right processes ${throughput.toLocaleString('en-US')} t/year at expected incremental margin $${margin}/t. Annual contribution before fixed fees?`,throughput*margin,'$',`${throughput.toLocaleString('en-US')} × $${margin} = ${money(throughput*margin)}. Tolling separates the processing service from ownership of the material. The margin is yours and so is the risk.`); },
        ...
        R => { const advance=r(R,5,25)*1000000, delivered=one(R,[20,30,40,50,60]); const remaining=advance*(1-delivered/100); return numeric(`Prepayment advance ${money(advance)}; ${delivered}% of the financed delivery obligation has been performed pro rata. Simplified remaining exposure before collateral/recoveries?`,remaining,'$',`${money(advance)} × (1 − ${delivered}%) = ${money(remaining)}. A prepayment unwinds only as cargo arrives. Until then it is unsecured lending to a producer.`); },
        R => { const cargo=r(R,10,50)*1000000, advance=one(R,[60,70,80]); const loan=cargo*advance/100; return numeric(`Eligible financed commodity value ${money(cargo)} with a ${advance}% advance rate. Simplified maximum funded amount?`,loan,'$',`${money(cargo)} × ${advance}% = ${money(loan)}. The advance rate is what actually funds the trade. The rest is your own capital, and it decides how much you can carry.`); },
        ...
        R => { const capex=r(R,20,100,10)*1000000, annual=r(R,5,25,5)*1000000; const payback=round(capex/annual,1); return numeric(`Project capex ${money(capex)} and expected annual cash contribution ${money(annual)}. Simple undiscounted payback period?`,payback,'years',`${money(capex)} ÷ ${money(annual)} = ${payback} years. Undiscounted payback is a first filter, not a valuation. It ignores the cost of the money, which on long projects dominates.`,.1); },
  ...
  EXTRA.a1.push(
    R => { const start=r(R,36,64,4), draw=r(R,4,16,2), days=30; const end=start-draw; const rate=round(draw/days,2); return numeric(`Stocks fall from ${start} Mt to ${end} Mt over ${days} days. Average inventory draw per day?`,rate,'Mt/day',`${draw} ÷ ${days} = ${rate} Mt/day. A draw rate turns a stock level into a countdown. It is what tells you whether a deficit is urgent or merely present.`,.01); },
    R => { const near=r(R,680,840,10), spread=r(R,-35,35,5), deferred=near-spread; return numeric(`Nearby futures are $${near}/t and deferred futures are $${deferred}/t. What is nearby minus deferred? Use a negative number for contango.`,spread,'$/t',`${near} − ${deferred} = $${spread}/t. A positive number is backwardation and a negative one contango. The sign alone tells you whether the market wants the goods now.`); },
    R => { const a=r(R,700,900,10), fa=r(R,20,55,5), b=a+r(R,-30,40,10), fb=r(R,20,55,5); const nA=a-fa, nB=b-fb, diff=nA-nB; return numeric(`Destination A pays $${a}/t with $${fa}/t freight. Destination B pays $${b}/t with $${fb}/t freight. By how much does A's netback exceed B's?`,diff,'$/t',`(${a} − ${fa}) − (${b} − ${fb}) = $${diff}/t. Two destinations are only comparable after freight. A higher headline price with a longer route can be the worse trade.`); },
    R => { const feed=r(R,620,780,10), yieldPct=r(R,82,94,2), product=r(R,760,940,10), cost=r(R,20,50,5); const rev=product*yieldPct/100; const margin=round(rev-feed-cost,1); return numeric(`One tonne of feed costs $${feed}. Saleable yield is ${yieldPct}% and product value is $${product}/t. Processing cost is $${cost}/t feed. Simple margin per tonne of feed?`,margin,'$/t',`${product} × ${yieldPct}% − ${feed} − ${cost} = $${margin}/t feed. Yield turns a feed price into a product margin, and a negative result is a real signal: at these prices the plant should cut runs.`,.1); },
    R => { const demand=r(R,90,130), supply=demand+r(R,-5,5), outage=r(R,2,6), demandRev=r(R,-3,3); const newBal=(supply-outage)-(demand+demandRev); return numeric(`Initial demand is ${demand} Mt and supply ${supply} Mt. A ${outage} Mt supply outage occurs and demand is revised by ${demandRev>=0?'+':''}${demandRev} Mt. New balance (supply − demand)?`,newBal,'Mt',`(${supply} − ${outage}) − (${demand} ${demandRev>=0?'+':'−'} ${Math.abs(demandRev)}) = ${newBal} Mt. A balance is the net of several moving parts, and an outage on one side is often partly absorbed by the other. Netting them is the whole exercise.`); },
    ...
    R => { const spread=r(R,12,36,4), storage=r(R,3,10), finance=r(R,2,8); const net=spread-storage-finance; return numeric(`Deferred value is $${spread}/t above prompt. Storage costs $${storage}/t and finance $${finance}/t. Net carry before losses?`,net,'$/t',`${spread} − ${storage} − ${finance} = $${net}/t. Storage is only a trade when the deferred premium beats the full carry, and the carry rises with the flat price.`); },
  ...
  EXTRA.a2.push(
    ...
    R => { const a=r(R,18,42,2), b=a+r(R,-8,8,2), qty=r(R,20,60,10)*1000; const diff=(a-b)*qty; return numeric(`Origin A gives a delivered margin of $${a}/t and Origin B $${b}/t on ${qty.toLocaleString('en-US')} t. Contribution advantage of A over B?`,diff,'$',`(${a} − ${b}) × ${qty.toLocaleString('en-US')} = ${money(diff)}. Origins are chosen on delivered contribution, not on the price at load port. The difference here is what the choice is worth.`); },
    ...
    R => { const m1=r(R,700,900,10), m2=m1+r(R,-30,30,5), qty=r(R,20,60,10)*1000; const pnl=(m2-m1)*qty; return numeric(`You are economically long the Month-2 versus Month-1 spread on ${qty.toLocaleString('en-US')} t. Month 1 settles $${m1}/t and Month 2 $${m2}/t. Spread P&L?`,pnl,'$',`(${m2} − ${m1}) × ${qty.toLocaleString('en-US')} = ${money(pnl)}. Being long a spread means you gain when it widens. When it moves the other way the loss is real even though the flat price never mattered.`); },
    ...
    R => { const gross=r(R,25,55,5), finance=r(R,3,10), credit=r(R,1,8), ops=r(R,2,9); const net=gross-finance-credit-ops; return numeric(`Headline margin $${gross}/t, finance $${finance}/t, expected credit cost $${credit}/t and execution reserve $${ops}/t. Risk-adjusted margin?`,net,'$/t',`${gross} − ${finance} − ${credit} − ${ops} = $${net}/t. Headline margin is the starting point, not the answer. Finance, credit and execution are what stand between it and cash.`); }
  );
  ...
  EXTRA.a3.push(
    ...
    R => { const wait=r(R,1,5), hire=r(R,16,36,2)*1000; return numeric(`A time-chartered vessel waits ${wait} days before the agreed loading window. Hire is ${money(hire)}/day. Direct waiting-hire cost?`,wait*hire,'$',`${wait} × ${money(hire)} = ${money(wait*hire)}. Waiting on hire is money spent before the cargo exists. It is the cost of a laycan agreed more tightly than the operation can deliver.`); },
    R => { const eta=r(R,1,3), window=r(R,2*eta+2,12); const slack=window-2*eta; return numeric(`A laycan window spans ${window} days. Your vessel ETA uncertainty is ±${eta} days. Ignoring other delays, how many days of nominal window remain beyond the two-sided ETA uncertainty?`,slack,'days',`${window} − 2 × ${eta} = ${slack} days. A window narrower than your own uncertainty is a promise you may not keep. Owners price that risk into the rate.`); },
    ...
    R => { const days=r(R,4,12), cons=r(R,20,45,5), fuel=r(R,450,750,50); const cost=days*cons*fuel; return numeric(`A vessel ballasts ${days} days, consuming ${cons} t/day of fuel at $${fuel}/t. Ballast bunker cost?`,cost,'$',`${days} × ${cons} × $${fuel} = ${money(cost)}. You pay for the ballast leg inside the freight rate. A cargo that suits where the ship already is will always be cheaper.`); },
    R => { const laden=r(R,12,28,4), ballast=r(R,4,12,2), port=r(R,4,8); const total=laden+ballast+port; return numeric(`Route requires ${laden} laden days, ${ballast} ballast days and ${port} port days. Total voyage duration?`,total,'days',`${laden} + ${ballast} + ${port} = ${total} days. Days are the currency of chartering. Every day is a day the owner cannot earn elsewhere, and it comes back to you in the rate.`); },
    ...
    R => { const flat=one(R,[18,22,26,30]), ws=r(R,70,160,10), qty=r(R,60,120,10)*1000; const rate=round(flat*ws/100,2), total=round(rate*qty); return numeric(`Worldscale flat rate is $${flat}/t and the fixture is WS${ws} on ${qty.toLocaleString('en-US')} t. Approximate freight?`,total,'$',`$${flat}/t × ${ws}% × ${qty.toLocaleString('en-US')} t = ${money(total)}. Worldscale points are a percentage of a published flat rate, so a flat rate revision moves your cost with no market move at all.`); }
  );
  ...
  EXTRA.a4.push(
    ...
    R => { const face=r(R,3,15,2)*1000000, disc=r(R,3,8), days=r(R,30,120,30); const cost=round(face*(disc/100)*(days/360)); return numeric(`A bank discounts a ${money(face)} deferred-payment LC at ${disc}% p.a. for ${days} days on a 360-day basis. Approximate discount cost?`,cost,'$',`${money(face)} × ${disc}% × ${days}/360 = ${money(cost)}. Discounting turns a deferred payment into cash today at a price. That price should be compared with the margin, not assumed.`); },
    R => { const qty=r(R,10,40,5)*1000, price=r(R,500,900,50), move=r(R,5,20,5); const add=qty*move; return numeric(`You sold ${qty.toLocaleString('en-US')} t open account. The mark-to-market moves $${move}/t in your favour before payment, increasing replacement exposure to the buyer. Additional exposure?`,add,'$',`${qty.toLocaleString('en-US')} × $${move} = ${money(add)}. Open account means the exposure grows with the market in your favour. A gain you have not been paid is credit risk, not profit.`); },
    ...
    R => { const inv=r(R,20,60,10), rec=r(R,20,80,10), pay=r(R,10,50,10); const cycle=inv+rec-pay; return numeric(`Inventory days ${inv}, receivable days ${rec}, supplier payable days ${pay}. Approximate cash conversion cycle?`,cycle,'days',`${inv} + ${rec} − ${pay} = ${cycle} days. The cash conversion cycle is the real constraint on volume. Payables are the only part that works for you.`); },
    ...
    R => { const exp=r(R,2,12,2)*1000000, pd=one(R,[1,2,3,4,5]), lgd=one(R,[30,40,50,60]); const el=round(exp*(pd/100)*(lgd/100)); return numeric(`Exposure ${money(exp)}, one-year default probability ${pd}%, loss-given-default ${lgd}%. Simplified expected loss?`,el,'$',`${money(exp)} × ${pd}% × ${lgd}% = ${money(el)}. Expected loss is what credit limits are sized against. It is a budget, not a forecast of any particular default.`); },
    R => { const gross=r(R,25,55,5), finance=r(R,4,12,2), el=r(R,1,8), capital=r(R,1,5); const net=gross-finance-el-capital; return numeric(`Gross margin $${gross}/t, finance $${finance}/t, expected credit cost $${el}/t and capital charge $${capital}/t. Credit-adjusted margin?`,net,'$/t',`${gross} − ${finance} − ${el} − ${capital} = $${net}/t. Risk-adjusted margin is the only comparison that treats a big safe trade and a small risky one honestly.`); }
  );
  ...
  EXTRA.a5.push(
    ...
    R => { const physical=r(R,4,12)*1000000, hedge=r(R,3,11)*1000000; const net=physical-hedge; return numeric(`Physical leg gains ${money(physical)} while hedge loses ${money(hedge)}. Combined hedge result?`,net,'$',`${money(physical)} − ${money(hedge)} = ${money(net)}. A hedge is judged on the combined result, not on either leg. A losing paper leg with a winning cargo is the hedge working.`); },
  ...
  EXTRA.a6.push(
    ...
    R => { const qty=r(R,10,80,5)*1000, lot=one(R,[25,50,100]); const lots=Math.round(qty/lot); return numeric(`Exposure is ${qty.toLocaleString('en-US')} t and the hedge contract is ${lot} t per lot. Approximate whole lots required?`,lots,'lots',`${qty.toLocaleString('en-US')} ÷ ${lot} ≈ ${lots} lots. Lot size forces rounding, and the rounding is itself a small unhedged position. It is why perfect hedges do not exist.`); },
    ...
    R => { const qty=r(R,20,70,10)*1000, phys=r(R,6,20,2), proxy=phys-r(R,-6,6,2); const residual=(phys-proxy)*qty; return numeric(`Physical value changes $${phys}/t and the proxy hedge offsets $${proxy}/t on ${qty.toLocaleString('en-US')} t. Residual move?`,residual,'$',`(${phys} − ${proxy}) × ${qty.toLocaleString('en-US')} = ${money(residual)}. A proxy hedge never offsets exactly. The residual is basis risk, and it is what you still own after the hedge is on.`); },
    R => { const fixed=r(R,70,110,5), floating=fixed+r(R,-15,15,5), qty=r(R,20,80,10)*1000; const settle=(floating-fixed)*qty; return numeric(`You receive floating and pay fixed $${fixed}/t in a swap on ${qty.toLocaleString('en-US')} t. Floating settles at $${floating}/t. Swap settlement?`,settle,'$',`(${floating} − ${fixed}) × ${qty.toLocaleString('en-US')} = ${money(settle)}. A swap settles the difference between floating and fixed. Paying fixed loses when the floating leg settles below it, which is the price of certainty.`); },
    R => { const fixed=r(R,70,110,5), avg=fixed+r(R,-12,12,3), qty=r(R,20,80,10)*1000; const settle=(avg-fixed)*qty; return numeric(`Monthly-average swap: receive average, pay fixed $${fixed}/t on ${qty.toLocaleString('en-US')} t. Monthly average is $${avg}/t. Settlement?`,settle,'$',`(${avg} − ${fixed}) × ${qty.toLocaleString('en-US')} = ${money(settle)}. Averaging is what makes a swap fit a quotational period. The settlement is against the average, not against any single day.`); },
    R => { const qty=r(R,10,50,5)*1000, delta=one(R,[0.25,0.4,0.55,0.7]), lot=one(R,[25,50,100]); const lots=Math.round(qty*delta/lot); return numeric(`Options cover ${qty.toLocaleString('en-US')} t with delta ${delta}. Futures lot size is ${lot} t. Approximate delta-hedge lots?`,lots,'lots',`${qty.toLocaleString('en-US')} × ${delta} ÷ ${lot} ≈ ${lots} lots. Delta says how much of the underlying the option currently behaves like, and it changes as the market moves.`); },
    R => { const exposure=r(R,20,80,10)*1000, ratio=one(R,[0.7,0.8,0.9,1.0]); const hedge=round(exposure*ratio); return numeric(`Physical exposure is ${exposure.toLocaleString('en-US')} t and chosen hedge ratio is ${ratio}. Hedge tonnage?`,hedge,'t',`${exposure.toLocaleString('en-US')} × ${ratio} = ${hedge.toLocaleString('en-US')} t. The first question is not how much to hedge but how much is genuinely unpriced. A ratio applied to the wrong base is a new position.`); },
  ...
  EXTRA.a7.push(
    ...
    R => { const dmt=r(R,5,15)*1000, grade=one(R,[20,22,24,26,28]), payable=one(R,[95,96,97]); const metal=round(dmt*grade/100*payable/100,1); return numeric(`Concentrate is ${dmt.toLocaleString('en-US')} dmt at ${grade}% copper with ${payable}% payable metal. Approximate payable copper tonnes?`,metal,'t',`${dmt.toLocaleString('en-US')} × ${grade}% × ${payable}% = ${metal.toLocaleString('en-US')} t. You are paid for the payable metal, not the tonnage. Grade and payability decide what a concentrate cargo is actually worth.`,.1); },
    R => { const qty=r(R,500,3000,250), lot=25; const lots=Math.round(qty/lot); return numeric(`Copper exposure is ${qty.toLocaleString('en-US')} t. Standard LME Copper lot size is ${lot} t. Approximate whole lots?`,lots,'lots',`${qty.toLocaleString('en-US')} ÷ ${lot} ≈ ${lots} lots. Exchange lots are fixed sizes. What does not fit into whole lots stays unhedged, and that remainder should be deliberate.`); },
    ...
    R => { const buy=r(R,90,180,10), sell=buy+r(R,40,120,10), freight=r(R,15,45,5), finance=r(R,5,20,5); const m=sell-buy-freight-finance; return numeric(`Buy premium $${buy}/t, sell premium $${sell}/t, freight $${freight}/t, finance $${finance}/t. Net physical premium margin?`,m,'$/t',`${sell} − ${buy} − ${freight} − ${finance} = $${m}/t. The premium is the metals desk's margin. Freight and finance eat it in exactly the same way they eat any other spread.`); },
  ...
  EXTRA.a8.push(
    ...
    R => { const prompt=r(R,70,100), deferred=prompt+r(R,-8,8); const spread=prompt-deferred; return numeric(`Prompt crude is $${prompt}/bbl and deferred crude $${deferred}/bbl. Prompt minus deferred?`,spread,'$/bbl',`${prompt} − ${deferred} = $${spread}/bbl. A positive number is backwardation and a negative one contango. The sign tells you whether the market is paying you to wait.`); },
    R => { const contango=r(R,2,8), storage=one(R,[0.6,0.8,1.0,1.2]), finance=one(R,[0.4,0.6,0.8,1.0]), loss=one(R,[0.1,0.2,0.3]); const net=round(contango-storage-finance-loss,1); return numeric(`Forward premium $${contango}/bbl; storage $${storage}, finance $${finance}, losses $${loss}/bbl. Net carry?`,net,'$/bbl',`${contango} − ${storage} − ${finance} − ${loss} = $${net}/bbl. Net carry is what decides whether floating or onshore storage is a trade. If it is negative the spread is not paying you to wait.`,.1); },
    ...
    R => { const buyAvg=r(R,70,100), sellAvg=buyAvg+r(R,-4,6); const qty=r(R,300,900,100)*1000; const pnl=(sellAvg-buyAvg)*qty; return numeric(`Purchase QP averages $${buyAvg}/bbl and sale QP averages $${sellAvg}/bbl on ${qty.toLocaleString('en-US')} bbl. Pricing-window contribution?`,pnl,'$',`(${sellAvg} − ${buyAvg}) × ${qty.toLocaleString('en-US')} = ${money(pnl)}. A pricing-window mismatch is a calendar spread position you never chose to take. It is the classic way a matched book is not matched.`); },
    ...
    R => { const dest=r(R,90,125), freight=r(R,2,7), losses=one(R,[0.2,0.4,0.6]), terminal=one(R,[0.3,0.5,0.8]); const nb=round(dest-freight-losses-terminal,1); return numeric(`Destination value $${dest}/bbl, freight $${freight}, losses $${losses}, terminal costs $${terminal}/bbl. Origin netback?`,nb,'$/bbl',`${dest} − ${freight} − ${losses} − ${terminal} = $${nb}/bbl. The netback decides where the barrel goes. Two destinations are only comparable after the whole route is paid for.`,.1); }
  );

  EXTRA.a9.push(
    R => { const brent=r(R,60,100), slope=one(R,[0.11,0.12,0.13,0.14]), constTerm=one(R,[0.2,0.5,0.8]); const price=round(brent*slope+constTerm,2); return numeric(`Oil-indexed LNG formula is ${slope} × Brent + $${constTerm}/MMBtu. Brent is $${brent}/bbl. Contract price?`,price,'$/MMBtu',`${slope} × ${brent} + ${constTerm} = $${price}/MMBtu. Oil indexation and hub pricing answer different questions. When they diverge, contracts get renegotiated.`,.01); },
    ...
    R => { const rate=r(R,80,180,20)*1000, days=r(R,20,40,5), energy=r(R,3,4)*1000000; const per=round(rate*days/energy,2); return numeric(`LNG vessel hire is ${money(rate)}/day for ${days} days. Cargo energy is ${energy.toLocaleString('en-US')} MMBtu. Hire cost per MMBtu before fuel?`,per,'$/MMBtu',`${money(rate)} × ${days} ÷ ${energy.toLocaleString('en-US')} = $${per}/MMBtu. Shipping cost per unit of energy is how an LNG voyage is compared with an alternative destination. Per day tells you nothing on its own.`,.01); },
    R => { const asia=r(R,15,25), fa=r(R,2,5), europe=asia+r(R,-4,4), fe=r(R,1,4); const diff=round((asia-fa)-(europe-fe),1); return numeric(`Asia value $${asia}/MMBtu with $${fa} logistics. Europe value $${europe}/MMBtu with $${fe} logistics. Asia netback minus Europe netback?`,diff,'$/MMBtu',`(${asia} − ${fa}) − (${europe} − ${fe}) = $${diff}/MMBtu. Delivered netbacks decide where a cargo goes. The higher headline index can easily be the lower netback once logistics are paid.`,.1); },
    ...
    R => { const winter=r(R,12,22), summer=winter-r(R,2,8), inject=one(R,[0.3,0.5,0.7]), withdraw=one(R,[0.2,0.4,0.6]), finance=one(R,[0.2,0.4,0.6]); const net=round(winter-summer-inject-withdraw-finance,1); return numeric(`Winter gas $${winter}/MMBtu, summer $${summer}. Injection $${inject}, withdrawal $${withdraw}, finance $${finance}/MMBtu. Net seasonal carry?`,net,'$/MMBtu',`${winter} − ${summer} − ${inject} − ${withdraw} − ${finance} = $${net}/MMBtu. The seasonal spread has to cover injection, withdrawal and financing before storage is worth doing.`,.1); },
    R => { const cap=r(R,2,6)*1000000, spread=r(R,1,5), usage=one(R,[50,60,70,80]); const value=round(cap*usage/100*spread); return numeric(`Storage working capacity ${cap.toLocaleString('en-US')} MMBtu, expected usable cycle ${usage}% and seasonal spread $${spread}/MMBtu before costs. Gross spread value?`,value,'$',`${cap.toLocaleString('en-US')} × ${usage}% × $${spread} = ${money(value)}. Usable capacity is not booked capacity. Cycling assumptions decide whether a storage book delivers what it promised.`); },
    R => { const dest=r(R,15,24), source=r(R,7,13), ship=r(R,2,5), regas=one(R,[0.4,0.7,1.0]), boil=one(R,[0.2,0.4,0.6]); const m=round(dest-source-ship-regas-boil,1); return numeric(`Destination $${dest}/MMBtu, source $${source}, shipping $${ship}, regas $${regas}, boil-off allowance $${boil}/MMBtu. Simplified LNG arbitrage margin?`,m,'$/MMBtu',`${dest} − ${source} − ${ship} − ${regas} − ${boil} = $${m}/MMBtu. Every step of the LNG chain takes a slice, boil-off included. The netback is what is left for the cargo itself.`,.1); },
    R => { const energy=r(R,3,4)*1000000, loss=one(R,[0.5,0.8,1.0,1.2]), value=r(R,12,22); const cost=round(energy*loss/100*value); return numeric(`Cargo energy ${energy.toLocaleString('en-US')} MMBtu. Total voyage/handling loss is ${loss}% and replacement value is $${value}/MMBtu. Value of lost energy?`,cost,'$',`${energy.toLocaleString('en-US')} × ${loss}% × $${value} = ${money(cost)}. Losses are predictable and belong in the economics from the start, not in a claim afterwards.`); }
  );

  EXTRA.a10.push(
    R => { const peak=r(R,20,60,10), off=r(R,20,80,10), hp=r(R,4,8), ho=r(R,8,16,4); const mwh=peak*hp+off*ho; return numeric(`A shaped contract delivers ${peak} MW for ${hp} peak hours and ${off} MW for ${ho} off-peak hours. Total MWh?`,mwh,'MWh',`${peak} × ${hp} + ${off} × ${ho} = ${mwh} MWh. Peak and off-peak are different products with different marginal generators. Averaging them hides where the money is made.`); },
    R => { const load=r(R,80,140,10), hedge=r(R,70,130,10), hours=r(R,4,10); const residual=(load-hedge)*hours; return numeric(`Actual load averages ${load} MW for ${hours} hours while your baseload hedge is ${hedge} MW. Residual energy position?`,residual,'MWh',`(${load} − ${hedge}) × ${hours} = ${residual} MWh. A baseload hedge against a shaped load leaves a volume residual every hour. That residual settles at imbalance prices.`); },
    R => { const peakP=r(R,90,180,10), offP=r(R,30,90,10), peakM=r(R,20,60,10), offM=r(R,40,100,10); const avg=round((peakP*peakM+offP*offM)/(peakM+offM),1); return numeric(`Peak price €${peakP}/MWh on ${peakM} MWh; off-peak €${offP}/MWh on ${offM} MWh. Load-weighted average price?`,avg,'€/MWh',`(${peakP} × ${peakM} + ${offP} × ${offM}) ÷ (${peakM+offM}) = €${avg}/MWh. A load-weighted price is what you actually paid. A simple average flatters a shape you did not have.`,.1); },
    ...
    R => { const qty=r(R,20,100,20)*1000, spread=r(R,5,30,5); const pnl=qty*spread; return numeric(`You are long Zone A and short Zone B on ${qty.toLocaleString('en-US')} MWh. A-minus-B spread widens €${spread}/MWh in your favour. P&L?`,pnl,'€',`${qty.toLocaleString('en-US')} × €${spread} = €${pnl.toLocaleString('en-US')}. Locational spreads cannot be arbitraged physically without transmission rights. Trading them is a view on congestion.`); },
    R => { const flow=r(R,50,200,25), spread=r(R,5,30,5), hours=r(R,4,12,2); const rent=flow*spread*hours; return numeric(`A constrained interconnector carries ${flow} MW for ${hours} hours while the zonal price spread is €${spread}/MWh. Gross congestion value?`,rent,'€',`${flow} × ${hours} × €${spread} = €${rent.toLocaleString('en-US')}. A constrained interconnector is where the zonal spread becomes real money, and where the capacity right is worth having.`); },
    R => { const error=r(R,5,25), hours=r(R,2,8), price=r(R,100,350,25); const cost=error*hours*price; return numeric(`Renewable output is ${error} MW below forecast for ${hours} hours and replacement/imbalance energy costs €${price}/MWh. Gross shortfall cost?`,cost,'€',`${error} × ${hours} × €${price} = €${cost.toLocaleString('en-US')}. A renewables forecast error is a volume exposure settled at imbalance prices, which are set to be unattractive.`); },
    ...
    R => { const margin=r(R,10,40,5), mw=r(R,50,200,25), hours=r(R,4,12,2); const value=margin*mw*hours; return numeric(`A flexible plant has positive dispatch margin €${margin}/MWh for ${mw} MW across ${hours} hours. Gross dispatch contribution?`,value,'€',`${margin} × ${mw} × ${hours} = €${value.toLocaleString('en-US')}. A flexible plant is an option: it runs when the spread pays and stops when it does not. That right is what is being valued.`); }
  );

  EXTRA.a11.push(
    R => { const begin=r(R,20,50,5), prod=r(R,80,140,10), imp=r(R,5,20,5), use=r(R,70,120,10), exp=r(R,10,30,5); const end=begin+prod+imp-use-exp; return numeric(`Beginning stocks ${begin} Mt, production ${prod}, imports ${imp}, domestic use ${use}, exports ${exp} Mt. Ending stocks?`,end,'Mt',`${begin} + ${prod} + ${imp} − ${use} − ${exp} = ${end} Mt. A balance is an estimate built on reported data with revisions. It is directional, and treating it as exact is how people are confidently wrong.`); },
    R => { const stocks=r(R,18,45,3), use=r(R,90,180,15); const ratio=round(stocks/use*100,1); return numeric(`Ending stocks are ${stocks} Mt and annual use is ${use} Mt. Stocks-to-use ratio?`,ratio,'%',`${stocks} ÷ ${use} × 100 = ${ratio}%. Stocks-to-use is the comparison that makes crops of different sizes comparable across years. The level alone means little.`,.1); },
    ...
    R => { const rent=r(R,3,9), finance=r(R,2,7), shrink=one(R,[0.5,1.0,1.5,2.0]); const req=round(rent+finance+shrink,1); return numeric(`Storage $${rent}/t, finance $${finance}/t and expected shrink/quality cost $${shrink}/t. Minimum deferred premium needed to break even?`,req,'$/t',`${rent} + ${finance} + ${shrink} = $${req}/t. Carrying grain has to be paid for by the deferred premium, and shrink and quality loss are part of the cost.`,.1); },
    ...
    R => { const qty=r(R,20,70,10)*1000, freight=r(R,20,50,5); const total=qty*freight; return numeric(`Ocean freight is $${freight}/t on ${qty.toLocaleString('en-US')} t of grain. Total freight?`,total,'$',`${qty.toLocaleString('en-US')} × $${freight} = ${money(total)}. On a low-value commodity freight is a large share of the delivered price, which is why origin choice is a freight decision.`); },
    R => { const days=r(R,1,5), rate=r(R,15,30,3)*1000; const cost=days*rate; return numeric(`A grain vessel incurs ${days} days of demurrage at ${money(rate)}/day. Cost?`,cost,'$',`${days} × ${money(rate)} = ${money(cost)}. Demurrage on grain is charged per day and argued per hour. It comes straight out of a thin margin.`); },
    R => { const dest=r(R,250,360,10), freight=r(R,25,55,5), inland=r(R,8,22,2), quality=r(R,0,12,2); const nb=dest-freight-inland-quality; return numeric(`Destination value $${dest}/t; ocean freight $${freight}, inland logistics $${inland}, expected quality discount $${quality}/t. Origin netback?`,nb,'$/t',`${dest} − ${freight} − ${inland} − ${quality} = $${nb}/t. Delivered economics decide the trade. The quality discount belongs in the calculation, not in the discussion afterwards.`); },
    R => { const sell=r(R,280,380,10), buy=sell-r(R,25,60,5), freight=r(R,20,45,5), quality=r(R,0,10,2), finance=r(R,2,8,2); const m=sell-buy-freight-quality-finance; return numeric(`Sell $${sell}/t, buy $${buy}/t, freight $${freight}, quality allowance $${quality}, finance $${finance}/t. Net agri margin?`,m,'$/t',`${sell} − ${buy} − ${freight} − ${quality} − ${finance} = $${m}/t. Agricultural margins are thin and the deductions are many. The net is the only figure worth quoting.`); }
  );
  ...
  EXTRA.a12.push(
    ...
    R => { const fob=r(R,600,900,20), freight=r(R,20,60,5), insurance=one(R,[1,2,3,4]), cif=fob+freight+insurance; return numeric(`FOB value $${fob}/t, main freight $${freight}/t and insurance allowance $${insurance}/t. Simplified CIF equivalent?`,cif,'$/t',`${fob} + ${freight} + ${insurance} = $${cif}/t. Moving from FOB to CIF means you now carry freight and insurance, and an unfixed freight exposure sits inside your margin.`); },
    R => { const qty=r(R,20,70,10)*1000, freight=r(R,20,50,5), handling=r(R,3,10); const cost=qty*(freight+handling); return numeric(`A contract change makes you responsible for $${freight}/t main carriage and $${handling}/t extra handling on ${qty.toLocaleString('en-US')} t. Added cost?`,cost,'$',`${qty.toLocaleString('en-US')} × (${freight} + ${handling}) = ${money(cost)}. A change of term moves cost and risk without anybody renegotiating the price. That is why incoterms are read before they are agreed.`); },
    ...
    R => { const qty=r(R,20,70,10)*1000, m1=r(R,650,900,10), better=r(R,10,40,5), m2=m1-better; const value=qty*better; return numeric(`Buyer may choose the lower of Month 1 at $${m1}/t or Month 2 at $${m2}/t on ${qty.toLocaleString('en-US')} t. In this realised scenario, value transferred to the buyer versus being forced to Month 1?`,value,'$',`${qty.toLocaleString('en-US')} × ($${m1} − $${m2}) = ${money(value)}. An option given to the buyer is exercised against you by definition. Pricing the worst case is the only honest way to quote it.`); },
    R => { const base=r(R,40,100,10)*1000, option=10, repl=r(R,5,20); const extra=base*option/100; const cost=extra*repl; return numeric(`Offtake volume is ${base.toLocaleString('en-US')} t with +${option}% buyer option. Extra tonnes cost $${repl}/t above contract economics when exercised. Realised option cost?`,cost,'$',`${base.toLocaleString('en-US')} × ${option}% × $${repl} = ${money(cost)}. The extra tonnes are supplied at contract economics while replacement costs more. That difference is the price of the option you granted.`); },
    R => { const qty=r(R,20,70,10)*1000, late=r(R,1,8), rate=one(R,[0.15,0.2,0.25,0.3]); const ld=round(qty*late*rate); return numeric(`Contract provides liquidated damages of $${rate}/t/day on ${qty.toLocaleString('en-US')} t. Delivery is ${late} days late. Damages?`,ld,'$',`${qty.toLocaleString('en-US')} × ${late} × $${rate} = ${money(ld)}. Liquidated damages are agreed in advance, so nobody has to prove a loss. They accrue per day and are claimed hard.`); },
    ...
    R => { const annual=r(R,50,180,10)*1000, margin=r(R,6,18,2), yrs=r(R,3,7), risk=r(R,1,5); const gross=annual*margin*yrs, reserve=annual*risk*yrs, net=gross-reserve; return numeric(`Offtake ${annual.toLocaleString('en-US')} t/year for ${yrs} years at $${margin}/t gross contribution, with $${risk}/t risk reserve. Undiscounted risk-adjusted life contribution?`,net,'$',`${annual.toLocaleString('en-US')} × (${margin} − ${risk}) × ${yrs} = ${money(net)}. Structural flow is valued over the life of the contract, net of the reserve. It is why origination is worth more than a spot cargo.`); }
  );
  EXTRA.a1.push(
